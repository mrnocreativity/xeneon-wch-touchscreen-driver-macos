...
final class PairingAuthorityTests: XCTestCase {
    ...
    func testRequiresTwoCompleteFreshContactsFromSameController() {
        var challenge = PairingChallenge(readyAt: 1_000_000_000, generation: 7)
        XCTAssertEqual(challenge.consume(event(.down, x: 4_096, at: 900_000_000)), .waiting)
        XCTAssertEqual(challenge.consume(event(.up, x: 4_096, at: 1_100_000_000)), .waiting)
        XCTAssertEqual(challenge.consume(event(.down, x: 4_096, at: 2_000_000_000)), .waiting)
        XCTAssertEqual(challenge.consume(event(.up, x: 4_096, at: 2_080_000_000)), .nextTarget)
        challenge.readyAt = 2_100_000_000
        XCTAssertEqual(challenge.consume(event(.down, x: 12_287, at: 2_090_000_000)), .waiting)
        XCTAssertEqual(challenge.consume(event(.down, x: 12_287, at: 3_000_000_000)), .waiting)
        XCTAssertEqual(challenge.consume(event(.up, x: 12_287, at: 3_080_000_000)), .complete)
    }

    func testCompetingControllerCannotCompleteCalibration() {
        var challenge = PairingChallenge(readyAt: 0, generation: 1)
        _ = challenge.consume(event(.down, x: 4_096, at: 1_000_000_000))
        XCTAssertEqual(challenge.consume(event(.up, x: 4_096, at: 1_080_000_000, controller: 2)), .rejected)
    }
    ...
    func testWrongTargetAndFastNoiseAreRejected() {
        ...
        _ = challenge.consume(event(.down, x: 4_096, at: 1_000_000_000))
        XCTAssertEqual(challenge.consume(event(.up, x: 4_096, at: 1_001_000_000)), .rejected)
    }

    func testDraggingOffTargetAndLongHoldAreRejected() {
        var challenge = PairingChallenge(readyAt: 0, generation: 1)
        _ = challenge.consume(event(.down, x: 4_096, at: 1_000_000_000))
        XCTAssertEqual(challenge.consume(event(.move, x: 8_000, at: 1_080_000_000)), .rejected)
        challenge = PairingChallenge(readyAt: 0, generation: 1)
        _ = challenge.consume(event(.down, x: 4_096, at: 1_000_000_000))
        XCTAssertEqual(challenge.consume(event(.up, x: 4_096, at: 4_000_000_000)), .rejected)
    }
