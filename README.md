...
Ambiguous associations are trusted only while endpoint observation remains registered in the current driver process. Identical controllers with duplicate serials and displays with a zero EDID serial—such as the tested Prechen panels—require physical pairing after a driver restart, sleep/wake, reconnect, or genuine loss of endpoint observation. UI and scheduling delays alone do not discard these associations. Associations can restore across identity-loss boundaries only when both endpoints expose public hardware identities that remain unique. Upgrading from an older pairing schema or calibration revision requires the current physical flow once, including for hardware-scoped records.
...
Touch traffic never starts calibration or advances topology-stability observations. If an unresolved controller enters storm mode, the prompt closes once and pairing waits for all unresolved controllers to recover through the quiet-period check. Recovered samples within an ongoing storm cannot pair a display or reopen the prompt. Display rearrangement preserves storm evidence; an already-paired controller's storm does not restart another controller's calibration. After quiet recovery, pairing contacts must still pass the stricter recovery-probation filter.

The driver receives HID callbacks and checks endpoint inventories on a dedicated interactive run loop, independent of AppKit, gesture processing, and file logging. Independent AppKit and endpoint receipts are checked once per second. A receipt older than four seconds blocks routing, but a scheduling delay does not delete pairings. Recovery cancels old gestures, rejects queued reports older than 250 ms or predating recovery, and revalidates endpoints and callback revisions before restoring input. An untouched calibration prompt stays in place; an interrupted contact must be retried. Actual membership changes, sleep/wake, and incomplete display transactions still revoke ambiguous authority. No software can detect a physically indistinguishable endpoint swap that produces no observable notification or inventory change; `re-pair` provides explicit recovery when the physical association is wrong despite apparently unchanged state.
...
`status` returns JSON with each controller's state and reason, active display bounds, observation generation, heartbeat freshness, and calibration target. It also reports `appKitFresh`, `endpointsFresh`, `responsivenessPaused`, and `droppedFileLogMessages`; reading status never invalidates pairings. File logging uses a bounded asynchronous queue, dropping file copies under overload rather than blocking input (Unified Logging remains available). Input diagnostics include received-report, parsed-event, and validated-event counts, the latest event and report age, storm state, and input disposition. They distinguish missing reports from validation or calibration rejection. States are `waitingForHardware`, `needsPairing`, `calibrating`, `active`, and `suspended`; only `active` with an open routing gate can route input. Status includes local identifiers and touch coordinates, so redact it before sharing publicly.
...
Storm tracking requires a dominant, regularly supported path within a bounded sample window. Contradictory samples and competing paths count against acquisition; local motion prediction permits gradual turns without letting gaps expand the spatial gate indefinitely. Releases are confirmed after a short interval, and a hold may start dragging only while its track still has fresh support. Lost confidence cancels the gesture rather than completing a tap.

One GCD timer per affected controller checks raw silence once per second while no track is acquired. It switches to 20 ms checks only while a contact needs release confirmation or expiry handling; reports do not create individual timers. One second of raw silence ends the incident and enters recovery probation. Three clean completed contacts restore ordinary validation; those contacts remain usable and do not require another pairing flow. Idle probation has no timer. Active incidents retain five-second summaries.

Diagnostics retain a 256-entry coordinate/decision ring and a bounded frozen pre/post-trigger capture. At most one capture per controller per 30 seconds is encoded off the input queue and written through bounded asynchronous logging to `<fileLogPath>.storms`, with a maximum 1 MiB current file and one rotated backup. Operational summaries remain in the main log so coordinate captures cannot displace multi-day lifecycle history. `Storm capture v1` records parsed down/move/up coordinates, monotonic timestamps, filter decisions, emitted-event counts and cancellations; `raw_idle` records have no coordinates, and `timer` records denote delayed decisions. These are local diagnostic samples, not complete USB packet recordings. Redact captures before sharing publicly. Status additionally exposes `stormRecoveryProbation` and `filterDecision`.

Storm-mode thresholds are conservative initial defaults verified with synthetic regression traces, not calibrated against complete recordings of the September incidents. Coherent controller-generated noise can still be indistinguishable from a real finger. Physical storm testing is required to measure false gestures, missed touches and latency. See [Selective Storm Filtering](docs/plans/2026-09-14-selective-storm-filtering-design.md) for the current design and verification scope.

This protection was added after a panel controller was captured emitting a raw HID touch storm while the production driver was stopped. The reports contained genuine touch bits but rapidly changing coordinates at the controller's report rate, so they originated upstream of synthetic event generation and could not be distinguished by checking the button byte alone. See [Touch Storm Protection Design](docs/plans/2026-09-01-touch-storm-protection-design.md) for the captured evidence and [Active Storm Confidence Tracking Design](docs/plans/2026-09-02-active-storm-confidence-tracking-design.md) for the earlier confidence-tracking design.
...
- On identical panels without unique public serials, calibration after a restart, sleep/wake, reconnect, or genuine endpoint-observation loss is intentional. Scheduling delays alone should pause input without asking for new pairing. The driver will not guess from screen order or position.
